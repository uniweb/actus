...
impl ExtractedParams {
    ...
    ///
    /// ⇒ Use [`Self::get_bool_opt`] when a caller has declared a default. This
    ...
    pub fn get_bool(&self, name: &str) -> Result<bool, WebError> {
        Ok(self.get_bool_opt(name)?.unwrap_or(false))
    }
    ...
    pub fn get_bool_opt(&self, name: &str) -> Result<Option<bool>, WebError> {
        Ok(self
            .scalar(name)
            .map(|s| !s.is_empty() && s != "false" && s != "0"))
    }
...
mod bool_default_tests {
    ...
    fn an_absent_bool_is_distinguishable_from_an_explicit_false() {
        ...
        let d = true;
        assert!(
            absent.get_bool_opt("flag").unwrap().unwrap_or(d),
